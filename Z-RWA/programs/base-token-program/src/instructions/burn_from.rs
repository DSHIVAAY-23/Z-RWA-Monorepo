...
/// Arguements:-
///   - name: token name
///   - to_account: target account where the token is going to be burned
...
///   - Signer does not belong to sub_admins, issuer and tokenization agent
///   - Amount is frozen
///   
/// Emits burn event
pub fn burn_from(ctx: Context<BurnTokenFrom>, params: TokenParams) -> Result<()> {
    ...
    if ctx.accounts.from.amount > 0 {
        let frozen_amount = ctx.accounts.partial_freeze.amount;
        require!(
            (ctx.accounts.from.amount - params.amount) >= frozen_amount,
