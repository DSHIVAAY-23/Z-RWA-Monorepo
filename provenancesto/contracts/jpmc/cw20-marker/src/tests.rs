...
fn create_marker() {
    ...
    // Verifying accesses
    assert!(is_issuer(&deps.as_mut().clone(), Addr::unchecked("issuer")).is_ok());
...
fn test_manage_signers() {
    ...
    let denom_multisig_info = DENOM_MULTISIG_INFO
        .load(deps.as_ref().storage.clone())
        .unwrap();
    assert_eq!(
...
fn test_manage_request() {
    ...
    let proposal_id = NONCE.load(deps.as_ref().storage.clone()).unwrap();
